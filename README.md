...
*Combine CMS is under development, and may or may not work for you :D*

--

Combine is a fork of jgrossi/corcel

Combine is a class collection created to retrieve Wordpress database data using a better syntax. It uses the Eloquent ORM developed for the Laravel Framework, but you can use Combine in any type of PHP project.
...
To install Combine just create a `composer.json` file and add:

    "require": {
        "ryanwhetstone/combine": "dev-master"
    },
...
Now you must set your Wordpress database params:
    ...
    Combine\Database::connect($params);
...
Updating post custom fields:
    ...
    $post = Post::find(1);
    $post->meta->username = 'ryanwhetstone';
    $post->meta->url = 'http://google.com';
    $post->save();
...
You can work with custom post types too. You can use the `type(string)` method or create your own class.
    ...
    class Video extends Combine\Post
    {
...
Combine is licensed under the MIT license.
